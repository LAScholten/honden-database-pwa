...
class ReuTeefCombinatie {
    ...
    async renderFuturePuppyPedigree(futurePuppy) {
        ...
        // Genereer de cards - pup card is nu klikbaar
        const mainDogCard = await this.generateDogCard(pedigreeTree.mainDog, this.t('futurePuppyName'), true, 0, true);
        const fatherCard = await this.generateDogCard(pedigreeTree.father, this.t('fatherLabel'), false, 1, true);
        const motherCard = await this.generateDogCard(pedigreeTree.mother, this.t('motherLabel'), false, 1, true);
        
        // Grootouders
        const paternalGrandfatherCard = await this.generateDogCard(pedigreeTree.paternalGrandfather, this.t('grandfatherLabel'), false, 2, true);
        const paternalGrandmotherCard = await this.generateDogCard(pedigreeTree.paternalGrandmother, this.t('grandmotherLabel'), false, 2, true);
        const maternalGrandfatherCard = await this.generateDogCard(pedigreeTree.maternalGrandfather, this.t('grandfatherLabel'), false, 2, true);
        const maternalGrandmotherCard = await this.generateDogCard(pedigreeTree.maternalGrandmother, this.t('grandmotherLabel'), false, 2, true);
        
        // Overgrootouders
        const paternalGreatGrandfather1Card = await this.generateDogCard(pedigreeTree.paternalGreatGrandfather1, this.t('greatGrandfatherLabel'), false, 3, true);
        const paternalGreatGrandmother1Card = await this.generateDogCard(pedigreeTree.paternalGreatGrandmother1, this.t('greatGrandmotherLabel'), false, 3, true);
        const paternalGreatGrandfather2Card = await this.generateDogCard(pedigreeTree.paternalGreatGrandfather2, this.t('greatGrandfatherLabel'), false, 3, true);
        const paternalGreatGrandmother2Card = await this.generateDogCard(pedigreeTree.paternalGreatGrandmother2, this.t('greatGrandmotherLabel'), false, 3, true);
        const maternalGreatGrandfather1Card = await this.generateDogCard(pedigreeTree.maternalGreatGrandfather1, this.t('greatGrandfatherLabel'), false, 3, true);
        const maternalGreatGrandmother1Card = await this.generateDogCard(pedigreeTree.maternalGreatGrandmother1, this.t('greatGrandmotherLabel'), false, 3, true);
        const maternalGreatGrandfather2Card = await this.generateDogCard(pedigreeTree.maternalGreatGrandfather2, this.t('greatGrandfatherLabel'), false, 3, true);
        const maternalGreatGrandmother2Card = await this.generateDogCard(pedigreeTree.maternalGreatGrandmother2, this.t('greatGrandmotherLabel'), false, 3, true);
    ...
    async generateDogCard(dog, relation, isMainDog = false, generation = 0, isClickable = true) {
        if (!dog) {
            return `
        ...
        // Voor toekomstige pup: speciale styling
        const isFuturePuppy = dog.id === -1;
        const futurePuppyClass = isFuturePuppy ? 'future-puppy' : '';
        const futurePuppyBorder = isFuturePuppy ? 'style="border-left: 4px solid #198754 !important;"' : '';
        
        return `
            <div class="pedigree-card-compact horizontal ${dog.geslacht === 'reuen' ? 'male' : 'female'} ${mainDogClass} ${futurePuppyClass} ${clickableClass} gen${generation}" 
                 data-dog-id="${dog.id}" 
        ...
        cards.forEach(card => {
            card.addEventListener('click', async (e) => {
                ...
                if (isFuturePuppy) {
                    ...
                    // Toon details van de toekomstige pup (fallback)
                    dog = pedigreeTree.mainDog;
                    await this.showFuturePuppyDetails(dog, relation);
                } else {

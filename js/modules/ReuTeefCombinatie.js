...
class ReuTeefCombinatie {
    ...
    showFuturePuppyPopup(futurePuppy, coiResult, healthAnalysis) {
        console.log('Toon toekomstige pup popup met eigen systeem');
    ...
    async renderFuturePuppyPedigree(futurePuppy) {
        ...
        try {
            ...
            // GEBRUIK EEN SYNCHRONE VERSIE - BELANGRIJKE VERBETERING
            const mainDogCard = this.generateDogCardSync(pedigreeTree.mainDog, this.t('mainDog'), true, 0);
            const fatherCard = this.generateDogCardSync(pedigreeTree.father, this.t('fatherLabel'), false, 1);
            const motherCard = this.generateDogCardSync(pedigreeTree.mother, this.t('motherLabel'), false, 1);
            
            const paternalGrandfatherCard = this.generateDogCardSync(pedigreeTree.paternalGrandfather, this.t('grandfatherLabel'), false, 2);
            const paternalGrandmotherCard = this.generateDogCardSync(pedigreeTree.paternalGrandmother, this.t('grandmotherLabel'), false, 2);
            const maternalGrandfatherCard = this.generateDogCardSync(pedigreeTree.maternalGrandfather, this.t('grandfatherLabel'), false, 2);
            const maternalGrandmotherCard = this.generateDogCardSync(pedigreeTree.maternalGrandmother, this.t('grandmotherLabel'), false, 2);
            
            const paternalGreatGrandfather1Card = this.generateDogCardSync(pedigreeTree.paternalGreatGrandfather1, this.t('greatGrandfatherLabel'), false, 3);
            const paternalGreatGrandmother1Card = this.generateDogCardSync(pedigreeTree.paternalGreatGrandmother1, this.t('greatGrandmotherLabel'), false, 3);
            const paternalGreatGrandfather2Card = this.generateDogCardSync(pedigreeTree.paternalGreatGrandfather2, this.t('greatGrandfatherLabel'), false, 3);
            const paternalGreatGrandmother2Card = this.generateDogCardSync(pedigreeTree.paternalGreatGrandmother2, this.t('greatGrandmotherLabel'), false, 3);
            const maternalGreatGrandfather1Card = this.generateDogCardSync(pedigreeTree.maternalGreatGrandfather1, this.t('greatGrandfatherLabel'), false, 3);
            const maternalGreatGrandmother1Card = this.generateDogCardSync(pedigreeTree.maternalGreatGrandmother1, this.t('greatGrandmotherLabel'), false, 3);
            const maternalGreatGrandfather2Card = this.generateDogCardSync(pedigreeTree.maternalGreatGrandfather2, this.t('greatGrandfatherLabel'), false, 3);
            const maternalGreatGrandmother2Card = this.generateDogCardSync(pedigreeTree.maternalGreatGrandmother2, this.t('greatGrandmotherLabel'), false, 3);
            ...
            // Voeg click events toe aan alle cards met EIGEN handlers
    ...
    // NIEUWE SYNCHRONE VERSIE - SNEL EN ZONDER ASYNC WACHTEN
    generateDogCardSync(dog, relation, isMainDog = false, generation = 0) {
        if (!dog) {
            return `
        ...
        // Standaard camera icon - GEEN ASYNC CHECKS HIER
        const cameraIcon = '';
    ...
    // OUD ASYNC VERSIE - WORDT ALLEEN GEBRUIKT VOOR POPUPS
    async generateDogCard(dog, relation, isMainDog = false, generation = 0) {
        ...
        // Check of deze hond foto's heeft - ASYNC MAAR WORDT ALLEEN GEBRUIKT IN POPUPS
    ...
    setupOwnCardClickEvents() {
        ...
        cards.forEach(card => {
            ...
            newCard.addEventListener('click', async (e) => {
                ...
                if (dogId === -999999) {
                    // Laat de speciale handler dit afhandelen
                    const futurePuppyCard = document.querySelector('.pedigree-card-compact.horizontal.main-dog-compact.gen0');
                    if (futurePuppyCard) {
                        futurePuppyCard.click();
                    }
                    return;
                }
